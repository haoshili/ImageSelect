...
import com.qiao.util.InternationalUtils;
 ...
 */
public class ImageSelectorFragment extends Fragment implements
    ...
    private TakePictureListenser listenser;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        initData();
        return rootView = inflater.inflate(R.layout.activity_image_selector,
                null);
    }
    ...
    private void initData() {
        ...
        if (intent.hasExtra("paramContext")) {
            paramContext = (SelectorParamContext) intent
    ...
    private void initViews() {
        ...
        dataList = (ArrayList<String>) intent.getSerializableExtra("datalist");
    ...
    private void pre() {
        Intent intent = DemoActivity.makeIntent(getActivity(),
                ImageBrowserFragment.class);
        List<String> list = paramContext.getSelectedFile();
        ArrayList<String> listString = new ArrayList<String>();
        for (String text : list) {
            String strs = "file://" + text;
            listString.add(strs);
        }
        intent.putExtra("dataList", listString);
    ...
    private void loadImages() {
        new Thread() {
            ...
            public void run() {
                ...
                final ArrayList<Bucket> bucketList = MediaHelper
                ...
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (paramContext.isMult()) {
                            findViewById(R.id.pic_browser_bottom)
    ...
    private void bindImageGridView(Cursor cursor) {
        if (imageAdapter != null) {
            ...
            // 第一次加载图片
            imageAdapter = new ImageCursorAdapter(getActivity(), cursor,
                    paramContext.getSelectedFile()) {
                @Override
    ...
    /**
     * 多张图片返回 path0,path1,path2..
     */
    private void onOKClick() {
        Intent intent = new Intent();
        // intent.putExtra("filePath",
        // getPathString(paramContext.getSelectedFile()));
        // intent.putExtra(SelectorParamContext.TAG_SELECTOR, paramContext);
        intent.putExtra("dataList",
                getShowFileList(paramContext.getSelectedFile()));
        // intent.putExtra("sb", 110);
        activity.setResult(activity.RESULT_OK, intent);
        activity.finish();
     ...
     * @return
     */
    private ArrayList<String> getShowFileList(ArrayList<String> fileList) {
        ArrayList<String> listFileShow = new ArrayList<String>();
        for (String file : fileList) {
            listFileShow.add("file://" + file);
        }
        return listFileShow;
    }
    ...
    private void bindBucketListView(ArrayList<Bucket> bucketList) {
        if (bucketList == null)
            return;
        albumListView.setAdapter(new BaseAdapter<Bucket>(getActivity(),
                bucketList) {
            @Override
            public View getView(final int position, View convertView,
                ...
                item.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        bindImageGridView(MediaHelper.getImagesCursor(context,
                                bucket.bucketId + ""));
                        hideAlbum();
                    }
                });
    ...
    private void refreshUi() {
        okView.setText(InternationalUtils.getString(R.string.complete_iamge,
                activity) + "(" + paramContext.getPercent() + ")");
        ...
        picQuality.setVisibility(paramContext.hasQulityMenu() ? View.VISIBLE
                : View.INVISIBLE);
    }
    ...
    private void popAlbum() {
        ...
        new AnimUtil(getActivity(), R.anim.translate_up_current)
                .setLinearInterpolator().startAnimation(albumRLayout);
    }
    ...
    private void hideAlbum() {
        new AnimUtil(getActivity(), R.anim.translate_down)
                .setLinearInterpolator().startAnimation(albumRLayout);
    ...
    public void loadImages(int start, int count) {
        for (int i = start + count - 1; i >= start; i--) {
            ...
            if (path != null && !path.trim().equals("")) {
                View view = imageGridView.findViewWithTag(CheckedImageView.TAG
                        + path);
                if (view != null) {
    ...
    public void setPictureListenser(TakePictureListenser listenser) {
